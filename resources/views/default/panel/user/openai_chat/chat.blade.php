...
@section('titlebar_actions')
    ...
    <x-button href="{{ LaravelLocalization::localizeUrl(route('dashboard.user.openai.list')) }}" class="ai-chat-action-btn">
...
@push('css')
    <style>
        /* AI Chat Page Background - Matching Chatbot Theme */
        ...
        /* Card Styling - Modern Gradient Background */
        body[data-theme="marketing-bot-dashboard"] .conversation-area-wrap,
        body[data-theme="marketing-bot-dashboard"] .lqd-card,
        body[data-theme="marketing-bot-dashboard"] #load_chat_area_container {
        ...
        /* Chat Sidebar Card Styling */
        body[data-theme="marketing-bot-dashboard"] .chats-sidebar-wrap .lqd-card,
        body[data-theme="marketing-bot-dashboard"] .chats-sidebar {
            background: linear-gradient(135deg, rgba(10, 14, 39, 0.95) 0%, rgba(26, 29, 58, 0.95) 50%, rgba(15, 23, 41, 0.95) 100%) !important;
            border: 1px solid rgba(0, 212, 255, 0.2) !important;
            backdrop-filter: blur(10px);
        }
        ...
        body[data-theme="marketing-bot-dashboard"] .lqd-icon {
            filter: drop-shadow(0 0 8px rgba(0, 212, 255, 0.6));
            transition: all 0.3s ease;
        }
        ...
        body[data-theme="marketing-bot-dashboard"] .lqd-chat-user-bubble .chat-content-container {
            background: linear-gradient(135deg, rgba(0, 212, 255, 0.2), rgba(123, 47, 247, 0.2)) !important;
            border: 1px solid rgba(0, 212, 255, 0.3) !important;
        }

        body[data-theme="marketing-bot-dashboard"] .lqd-chat-ai-bubble .chat-content-container {
...
@section('content')
    ...
    <div class="rocket-stars-background" id="rocket-stars-ai-chat"></div>
...
@push('script')
    ...
    <script>
        // Interactive Stars Background for AI Chat Page
        let aiChatStars = [];
        let aiChatMouseX = 0;
        let aiChatMouseY = 0;
        
        function createAIChatStars() {
            const starsContainer = document.getElementById('rocket-stars-ai-chat');
            ...
            document.addEventListener('mousemove', function(e) {
                ...
                aiChatMouseX = ((e.clientX - rect.left) / rect.width) * 100;
                aiChatMouseY = ((e.clientY - rect.top) / rect.height) * 100;
        ...
        function updateAIChatStars() {
            aiChatStars.forEach(star => {
                const starX = parseFloat(star.dataset.x);
                const starY = parseFloat(star.dataset.y);
                
                // Calculate distance from mouse
                const dx = aiChatMouseX - starX;
                const dy = aiChatMouseY - starY;
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', createAIChatStars);
        } else {
            createAIChatStars();
        }
    </script>
