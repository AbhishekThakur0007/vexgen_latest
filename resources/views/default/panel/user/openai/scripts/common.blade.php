<script>
    ...
    function sendOpenaiGeneratorForm(ev) {
        ...
        @if ($openai->type == 'image')
            ...
            if (imageGenerator === 'openai' || imageGenerator === 'gpt-image-1') {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                // Trigger XP notification check after generation
                // Wait a moment for XP to be awarded by the observer
                setTimeout(function() {
                    // Dispatch custom event for XP notification system
                    document.dispatchEvent(new CustomEvent('generation-completed', {
                        detail: { generationId: res.id, type: '{{ $openai->type }}' }
                    }));
                    
                    // Also directly check notifications if system exists
                    if (window.xpNotificationSystem && typeof window.xpNotificationSystem.checkNotifications === 'function') {
                        window.xpNotificationSystem.checkNotifications();
                    }
                }, 1000);
