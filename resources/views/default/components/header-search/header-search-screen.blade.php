...
>
    <div class="{{ @twMerge('header-search-input-wrap relative w-full', $attributes->get('class:input-wrap')) }}">
        @if ($showIcon)
            @php($commanderInlineSearchGradient = uniqid('commander-inline-search-gradient-'))
            <svg
                class="{{ @twMerge('header-search-icon commander-inline-search-icon pointer-events-none absolute start-3 top-1/2 z-10 -translate-y-1/2', $attributes->get('class:icon')) }}"
                viewBox="0 0 24 24"
                fill="none"
            ...
            >
                <defs>
                    <linearGradient id="{{ $commanderInlineSearchGradient }}" x1="5" y1="5" x2="19" y2="19" gradientUnits="userSpaceOnUse">
                ...
                <circle
                    ...
                    cy="11"
                    r="5.5"
                    stroke="url(#{{ $commanderInlineSearchGradient }})"
                    stroke-width="1.6"
                    fill="rgba(10, 18, 45, 0.4)"
            />
                <path
                    d="M15.5 15.5L19.2 19.2"
                    stroke="url(#{{ $commanderInlineSearchGradient }})"
                    stroke-width="1.6"
    ...
    @if ($style === 'modern')
        <template x-teleport="body">
            ...
            >
                ...
                <div
                    class="lqd-modal-content relative z-[100] max-h-[95vh] min-w-[clamp(250px,760px,90vw)] max-w-[min(calc(100%-2rem),630px)] overflow-y-auto overscroll-contain rounded-xl bg-gradient-to-br from-[#0a1a2d] via-[#1a2b4d] to-[#0f1e35] border border-[#00d4ff]/20 shadow-2xl shadow-[#00d4ff]/10 group-[.modal-open]/header-search-modal:motion-scale-in-[0.98] group-[.modal-open]/header-search-modal:motion-opacity-in-[0%] group-[.modal-open]/header-search-modal:motion-duration-200">
                    {{-- Search input --}}
                    <div class="relative flex items-center justify-between border-b border-[#00d4ff]/10">
