@php
    $container_class = ['header-search-results-container max-h-[min(440px,80vh)] overflow-y-auto rounded-xl bg-gradient-to-br from-[#0a1a2d] via-[#1a2b4d] to-[#0f1e35] shadow-xl shadow-black/20'];
...
>
    ...
    >
        <h3 class="m-0 border-b border-[#00d4ff]/10 py-4 ps-5 text-start text-base font-medium">
            {{ __('Search results') }}
        </h3>
    ...
    >
        <h3 class="m-0 w-full border-b border-[#00d4ff]/10 px-5 py-4 text-start text-base font-medium">
            {{ __('Recent Search') }}
        </h3>
        ...
        <x-button
            class="w-full justify-between rounded-none border-b border-[#00d4ff]/10 px-5 py-4 font-heading !text-base !font-medium text-heading-foreground"
