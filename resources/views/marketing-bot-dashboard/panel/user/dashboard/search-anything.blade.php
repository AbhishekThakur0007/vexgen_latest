<div
    ...
    <div class="relative z-1 mx-auto lg:w-7/12">
        <p class="mb-10 text-center font-heading text-[21px] font-bold leading-tight -tracking-wide">
            <span class="mx-auto mb-4 inline-flex items-center justify-center gap-3 rounded-full bg-foreground/10 px-6 py-2 text-xs uppercase tracking-[0.4em] text-foreground/70 shadow-[0_0_22px_rgba(123,47,247,0.35)]">
                🚀 {{ __('Mission Control Search') }}
            </span>
            <span class="create-anything-text block text-[51px] font-bold">
                {{ __('Create anything') }}
            </span>
            <span class="mt-4 block text-base font-normal text-foreground/60">
                {{ __('Plot your next launch, summon assets, or revisit campaigns from a single command console.') }}
            </span>
        </p>
        ...
        <div class="relative mb-8">
            <x-header-search
                ...
                class:input="h-20 rounded-full border-none bg-gradient-to-r from-[#0a1a2d] via-[#1a2b4d] to-[#0f1e35] ps-16 sm:ps-20 text-heading-foreground shadow-[0_4px_12px_rgba(0,212,255,0.15)] placeholder:text-heading-foreground/70 sm:text-xs placeholder-shown:text-ellipsis max-sm:pe-16"
            ...
            <span
                class="roudned-full pointer-events-none absolute end-3 top-1/2 inline-grid size-10 -translate-y-1/2 place-items-center rounded-full bg-gradient-to-br from-[#0a1a2d]/80 via-[#1a2b4d]/80 to-[#0f1e35]/80 backdrop-blur border border-[#00d4ff]/20 sm:end-4 sm:size-11"
            >
                {{-- blade-formatter-disable --}}
