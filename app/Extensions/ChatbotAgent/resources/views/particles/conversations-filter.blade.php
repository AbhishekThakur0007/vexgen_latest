<div class="relative flex h-[--header-height] shrink-0 flex-wrap items-center gap-2.5 border-b px-4 xl:px-6">
    <x-button
        @class([
            'size-8 bg-foreground/10 text-foreground lg:group-[&.focus-mode]/body:flex',
    ...
    >
        <x-slot:trigger
            class="gap-1 font-heading text-base font-bold text-heading-foreground sm:gap-2"
            variant="link"
        >
            <svg
                width="18"
            ...
            <span
                class="max-[330px]:hidden lg:max-xl:hidden"
                ...
                @foreach ($status_filters as $key => $filter)
                    <li>
                        <a
                            ...
                            href="#"
                            @click.prevent="filterStatus('{{ $key }}', '{{ $filter['label'] }}')"
