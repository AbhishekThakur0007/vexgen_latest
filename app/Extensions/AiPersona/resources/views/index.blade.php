...
@section('titlebar_title')
    <span class="ai-persona-page-title">
        {{ __('AI Persona') }}
    </span>
@endsection
@section('titlebar_subtitle')
    {{ __('Create studio-quality videos with AI Persona and voiceovers in 130+ languages. It\'s as easy as making a slide deck.') }}
@endsection
...
@push('css')
    <style>
        /* AI Persona Page Background - Matching Dashboard Theme */
...
@section('content')
    ...
    <div class="rocket-stars-background" id="rocket-stars-ai-persona"></div>
...
@push('script')
    <script>
        // Interactive Stars Background for AI Persona Page
        let aiPersonaStars = [];
        let aiPersonaMouseX = 0;
        let aiPersonaMouseY = 0;
        
        function createAiPersonaStars() {
            const starsContainer = document.getElementById('rocket-stars-ai-persona');
            ...
            document.addEventListener('mousemove', function(e) {
                ...
                aiPersonaMouseX = ((e.clientX - rect.left) / rect.width) * 100;
                aiPersonaMouseY = ((e.clientY - rect.top) / rect.height) * 100;
                
                updateAiPersonaStars();
            });
        ...
        function updateAiPersonaStars() {
            aiPersonaStars.forEach(star => {
                const starX = parseFloat(star.dataset.x);
                const starY = parseFloat(star.dataset.y);
                
                // Calculate distance from mouse
                const dx = aiPersonaMouseX - starX;
                const dy = aiPersonaMouseY - starY;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            createAiPersonaStars();
        });
        
        // Re-initialize if content is loaded dynamically
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', createAiPersonaStars);
        } else {
            createAiPersonaStars();
        }
    </script>
