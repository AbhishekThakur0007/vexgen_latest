...
@section('titlebar_title')
    <span class="telegram-group-page-title">
        {{ __('Telegram Groups') }}
    </span>
@endsection
...
@push('css')
    <style>
        /* Telegram Group Page Background - Matching Dashboard Theme */
...
@section('content')
    ...
    <div class="rocket-stars-background" id="rocket-stars-telegram-group"></div>
...
@push('script')
    <script>
        // Interactive Stars Background for Telegram Group Page
        let telegramGroupStars = [];
        let telegramGroupMouseX = 0;
        let telegramGroupMouseY = 0;
        
        function createTelegramGroupStars() {
            const starsContainer = document.getElementById('rocket-stars-telegram-group');
            ...
            document.addEventListener('mousemove', function(e) {
                ...
                telegramGroupMouseX = ((e.clientX - rect.left) / rect.width) * 100;
                telegramGroupMouseY = ((e.clientY - rect.top) / rect.height) * 100;
                
                updateTelegramGroupStars();
            });
        ...
        function updateTelegramGroupStars() {
            telegramGroupStars.forEach(star => {
                const starX = parseFloat(star.dataset.x);
                const starY = parseFloat(star.dataset.y);
                
                // Calculate distance from mouse
                const dx = telegramGroupMouseX - starX;
                const dy = telegramGroupMouseY - starY;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            createTelegramGroupStars();
        });
        
        // Re-initialize if content is loaded dynamically
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', createTelegramGroupStars);
        } else {
            createTelegramGroupStars();
        }
    </script>
